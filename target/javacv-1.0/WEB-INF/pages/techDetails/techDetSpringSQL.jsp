...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<div>
<p>Для каждого пользователя сайта(который зайдет на эту страницу) создается <br/> 
3 таблицы Products, Customers, Orders. Таблицы создаются в момент перехода  <br/> 
на страницу "Технические детали" (если не были созданы до этого),  а удаляеться <br/> 
либо после удаления пользователя либо после закрытия сессии (48 часов  <br/>
с момента неактивности). C точки зрения экономии ресурсов хранить 3 таблицы<br/>
для каждого пользователя ОЧЕНЬ неразумно, но я исхожу из того что на сайте <br/>
будет до 25 активных сессий одновременно, а это не критично <br/>
Содержимое таблиц можно менять. <br/> </p>

<form method="GET" action="showPage">
	<input name="direction" type="submit" value="Изменить таблицы">
</form> 
...
<p> SELECT c.id as customerId, name, o.id as orderId, product_id, customer_id <br/> 
 	...
 	<tbody>
 		<c:forEach items="${selectCustomerOrderCrossJoin}" var="list">
   			<tr>
  				...
  				<td><c:out value="${list.orderCustomerId}" /></td>
...
<p> SELECT c.id as customerId, name, o.id as orderId, product_id, customer_id
...
Все строки левой(та, которая до JOIN) таблицы, которые не нашли соответствия в правой все равно будут добавлены в результирующую таблицу.
<p> SELECT c.id as customerId, name, o.id as orderId, product_id, customer_id
	FROM Customers c LEFT OUTER JOIN Orders o ON(c.id = o.customer_id); </p><br/>
<table>
	<thead style="background:#fcf">
  		<tr>
   			<th>customerId</th>
 	...
 	<tbody>
 		<c:forEach items="${selectCustomerOrderLeftOuterJoin}" var="list">
   			<tr>
  				<td><c:out value="${list.customerId}" /></td>
  				<td><c:out value="${list.customerName}" /></td>
  				<td><c:out value="${list.orderId}" /></td>
